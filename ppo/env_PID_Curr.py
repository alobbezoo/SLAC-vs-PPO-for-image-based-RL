...
homeDir = os.getcwd()
imgDir = homeDir + "/ppo/trainingImagesEnvPIDCurr/"
...
class ArmEnv(object):
    ...
    def reset(self):
        ...
        if self.stepCount <= 5 * 10 ** 5: # intially training the planner (target a set radius away)
...
class Viewer(pyglet.window.Window):
    ...
    def render(self):
        ...
        # print("shape: ", np.shape(render3))
        # print("max: ", np.amax(render3))
        # print("min: ", np.amin(render3))
        # print("type state1: ", type(render3))
        render4 = np.moveaxis(render3, 2, 0) # (3, x, x) format for training

        # im = PIL.Image.fromarray(render3)
        # im.save(image_compress)
