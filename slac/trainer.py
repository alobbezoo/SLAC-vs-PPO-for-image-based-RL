...
class Trainer:
    ...
    def __init__(
        ...
        log_dir,
        seed=0,
        num_steps= 2 * 10 ** 3, #3 * 10 ** 6, NOTE: this 
        initial_collection_steps= 9 * 10 ** 3, # NOTE: Reduce for testing
        initial_learning_steps= 2 * 10 ** 3,
        num_sequences=8,
        eval_interval= 10 ** 3,
        num_eval_episodes=5,
    ):
    ...
    def evaluate(self, step_env): #NOTE noticed massive issues in the evaluate function. 
        mean_return = 0.0
        mean_return_array = np.array([])
        ...
        for i in range(self.num_eval_episodes):
            ...
            mean_return_array = np.append(arr = mean_return_array, values=episode_return)

        mean_return = np.mean(mean_return_array)
        #print("mean return: ", mean_return)
        
        # save value for when optuna_return is called
        self.optuna_mean_return = mean_return
    ...
    def optuna_return(self): 
        return self.optuna_mean_return
