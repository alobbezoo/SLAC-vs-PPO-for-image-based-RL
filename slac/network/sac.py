...
class GaussianPolicy(torch.jit.ScriptModule):
    ...
    def sample(self, feature_action):
        ...
        # clamp_log_std = log_std.clamp_(-20, 2) #removed the in place operator
        clamp_log_std = log_std.clamp(-20, 2) # out of place operator
        action, log_pi = reparameterize(mean, clamp_log_std)
